...
#include <mega8535.h>
unsigned char variableD;
unsigned char variableC;
...
while (1){
        ...
        //del puerto D ya que los demás no interesan.
        
        if (variableD<10)
        PORTB=tabla7segmentos[variableD];
        if (variableD>=10) //Si lo que leemos es mayor o igual de 10 que dibuje en el display una E de ERROR
        PORTB=0x79;
        
        if (variableC<10)
        PORTA=tabla7segmentos[variableC];
        if (variableC>=10) //Si lo que leemos es mayor o igual de 10 que dibuje en el display una E de ERROR
        PORTA=0x79;
