...
while (1){
    ...
    while(count){
        if (boton==1)
            ...
            botona=0;
        if ((botonp==0)&&(botona==1)){ //hubo cambio de flanco de 1 a 0
            var1+=9; //Se incrementa la variable
            delay_ms(40); //Se coloca retardo de 40mS para eliminar rebotes
        }
        if ((botonp==1 )&&(botona== 0)) //hubo cambio de flanco de 0 a 1
            delay_ms(40); //Se coloca retardo de 40mS para eliminar rebotes   
        botonp=botona;    
